...
#  to show data in website for ex: image of products
...
def add_to_cart(request):
 ...
 #  print(product) # print value for debug
 product = Product.objects.get(id=product_id) # to get instance
...
def plus_cart(request):
 if request.method == 'GET':
  ...
  c.quantity+=1 # to increase cart quantity
  c.save()
  amount = 0.0
  shipping_amount = 70.0
...
def top_wear(request, data=None):
  ...
  if data == None:
   ...
   topwears = Product.objects.filter(category='TW').filter(discounted_price__lt=200)
  elif data == 'above':
   topwears = Product.objects.filter(category='TW').filter(discounted_price__gt=200)
  if request.user.is_authenticated:
   totalitem = len(Cart.objects.filter(user=request.user)) 
...
def bottom_wear(request, data=None):
  ...
  if data == None:
   ...
   bottomwears = Product.objects.filter(category='BW').filter(discounted_price__lt=200)
  elif data == 'above':
   bottomwears = Product.objects.filter(category='BW').filter(discounted_price__gt=200)
  if request.user.is_authenticated:
   totalitem = len(Cart.objects.filter(user=request.user))  
...
#  to save data in the database
 def get(self, request):
  ...
  return render(request, 'app/profile.html', {'form':form, 'active':'btn-primary', 'totalitem':totalitem}) # active and also writen active to active profile
 def post(self, request):
